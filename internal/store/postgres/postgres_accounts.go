...
func (s *Store) GetPrimaryAccount(ctx context.Context, userID int) (*model.Account, error) {
	query := `
		SELECT id, user_id, account_type, name, is_active, created_at, updated_at
		FROM accounts
		WHERE user_id = $1 AND account_type = 'primary' AND is_active = 1
		LIMIT 1
	`
	account := &model.Account{}
...
func (s *Store) UpdateAccount(ctx context.Context, account *model.Account) error {
	query := `
		UPDATE accounts
		SET account_type = $1, name = $2, is_active = $3, updated_at = $4
		WHERE id = $5
	`
...
func (s *Store) UpdateSpendingLimit(ctx context.Context, limit *model.SpendingLimit) error {
	query := `
		...
		SET daily_limit = $1, weekly_limit = $2, monthly_limit = $3,
			daily_spent = $4, weekly_spent = $5, monthly_spent = $6,
			daily_reset_at = $7, weekly_reset_at = $8, monthly_reset_at = $9,
			is_daily_blocked = $10, is_weekly_blocked = $11, is_monthly_blocked = $12,
			updated_at = $13
		WHERE id = $14
	`
	_, err := s.db.ExecContext(ctx, query,
...
func (s *Store) ResetSpendingLimits(ctx context.Context) error {
	query := `
		...
		SET
			...
			weekly_spent = CASE
				WHEN NOW() - weekly_reset_at >= INTERVAL '7 days' THEN 0
				ELSE weekly_spent
			END,
			weekly_reset_at = CASE
				WHEN NOW() - weekly_reset_at >= INTERVAL '7 days' THEN $2
				ELSE weekly_reset_at
			END,
			is_weekly_blocked = CASE
				WHEN NOW() - weekly_reset_at >= INTERVAL '7 days' THEN FALSE
				ELSE is_weekly_blocked
			END,
			monthly_spent = CASE
				WHEN NOW() - monthly_reset_at >= INTERVAL '30 days' THEN 0
				ELSE monthly_spent
			END,
			monthly_reset_at = CASE
				WHEN NOW() - monthly_reset_at >= INTERVAL '30 days' THEN $3
				ELSE monthly_reset_at
			END,
			is_monthly_blocked = CASE
				WHEN NOW() - monthly_reset_at >= INTERVAL '30 days' THEN FALSE
				ELSE is_monthly_blocked
			END,
			updated_at = $4
		WHERE NOW() - daily_reset_at >= INTERVAL '1 day'
	`
	now := time.Now()
...
func (s *Store) GetPendingTransferRequests(ctx context.Context, householdID int) ([]*model.TransferRequest, error) {
	query := `
		...
		WHERE u.household_id = $1 AND tr.status = 'pending'
...
func (s *Store) UpdateTransferRequest(ctx context.Context, transfer *model.TransferRequest) error {
	query := `
		...
		SET status = $1, approved_at = $2, approved_by = $3, rejection_reason = $4, updated_at = $5
		WHERE id = $6
	`
	_, err := s.db.ExecContext(ctx, query,
		transfer.Status,
...
func (s *Store) RejectTransferRequest(ctx context.Context, transferID int, approvedBy int, reason string) error {
	...
	query := `
		UPDATE transfer_requests
		SET status = 'rejected', approved_at = $1, approved_by = $2, rejection_reason = $3, updated_at = $4
		WHERE id = $5
	`
...
func (s *Store) GetUsersEligibleForInterest(ctx context.Context) ([]*model.User, error) {
	query := `
		...
		WHERE interest_rate_annual > 0
			AND (last_interest_date IS NULL OR NOW() - last_interest_date >= INTERVAL '30 days')
	`
...
func (s *Store) UpdateUserInterestDate(ctx context.Context, userID int, date time.Time) error {
	query := `UPDATE users SET last_interest_date = $1, updated_at = $2 WHERE id = $3`
