...
func (s *Store) ResetSpendingLimits(ctx context.Context) error {
	query := `
		...
		SET
			...
			is_daily_blocked = 0,
			weekly_spent = CASE
				WHEN DATEDIFF(NOW(), weekly_reset_at) >= 7 THEN 0
				ELSE weekly_spent
			END,
			weekly_reset_at = CASE
				WHEN DATEDIFF(NOW(), weekly_reset_at) >= 7 THEN ?
			...
			END,
			monthly_spent = CASE
				WHEN DATEDIFF(NOW(), monthly_reset_at) >= 30 THEN 0
				ELSE monthly_spent
			END,
			monthly_reset_at = CASE
				WHEN DATEDIFF(NOW(), monthly_reset_at) >= 30 THEN ?
			...
			END,
			updated_at = ?
		WHERE DATEDIFF(NOW(), daily_reset_at) >= 1
	`
	now := time.Now()
...
func (s *Store) GetUsersEligibleForInterest(ctx context.Context) ([]*model.User, error) {
	query := `
		...
		WHERE interest_rate_annual > 0
			AND (last_interest_date IS NULL OR DATEDIFF(NOW(), last_interest_date) >= 30)
	`
